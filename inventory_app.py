...
class MainWindow(QMainWindow):
    ...
    def refresh_stock_list(self):
        ...
        for r in rows:
            ...
            self.stock_table.setItem(row, 4, qitem(qty_text))  # 在庫数
            self.stock_table.setItem(row, 5, qitem(safety_text))  # 安全在庫
    ...
    def _build_in_history_tab(self):
        ...
        self.in_hist_table = QTableWidget(0, 7)
        self.in_hist_table.setHorizontalHeaderLabels(
            ["日時", "コード", "備品名", "数量", "理由", "担当者", "メモ"]
        )
    ...
    def refresh_in_history(self):
        ...
        for r in rows:
            ...
            unit = r["unit"] or ""
            qty_text = f'{int(r["qty"])} {unit}'.strip()
            self.in_hist_table.setItem(row, 3, qitem(qty_text))
            self.in_hist_table.setItem(row, 4, qitem(r["reason"] or ""))
            self.in_hist_table.setItem(row, 5, qitem(r["user"] or ""))
            self.in_hist_table.setItem(row, 6, qitem(r["memo"] or ""))
    ...
    def export_in_history_csv(self):
        ...
        with open(path, "w", newline="", encoding="utf-8-sig") as f:
            w = csv.writer(f)
            w.writerow(["日時", "コード", "備品名", "数量", "単位", "理由", "担当者", "メモ"])
    ...
    def export_in_history_excel(self):
        ...
        headers = ["日時", "コード", "備品名", "数量", "単位", "理由", "担当者", "メモ"]
        ws.append(headers)
    ...
    def _build_out_history_tab(self):
        ...
        self.out_hist_table = QTableWidget(0, 7)
        self.out_hist_table.setHorizontalHeaderLabels(
            ["日時", "コード", "備品名", "数量", "理由", "納品先", "メモ"]
        )
    ...
    def export_out_history_csv(self):
        ...
        with open(path, "w", newline="", encoding="utf-8-sig") as f:
            w = csv.writer(f)
            w.writerow(["日時", "コード", "備品名", "数量", "単位", "理由", "納品先", "メモ"])
    ...
    def export_out_history_excel(self):
        ...
        headers = ["日時", "コード", "備品名", "数量", "単位", "理由", "納品先", "メモ"]
        ws.append(headers)
