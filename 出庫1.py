...
total_summary = (
    df.groupby(group_columns, as_index=False, dropna=False)["数量"]
    .sum()
...
summary = pd.merge(total_summary, monthly_pivot, on=group_columns, how="outer")

basic_columns = ["コード", "備品名", "総消耗数", "単位"]

month_columns = sorted(
    [
        column
        for column in summary.columns
        if column not in ["コード", "備品名", "単位", "総消耗数"]
    ]
)

summary = summary[basic_columns + month_columns]
summary[month_columns] = (summary[month_columns].replace(0, ""))
summary = summary.sort_values("コード",na_position="last")
